...
```text
ub_overflow_model_cpp/data/adapter/                 原始 adapter
ub_overflow_model_cpp/data/before_cvpipelining/     171 个当前 corpus MLIR
ub_overflow_model_cpp/data/before_cvpipelining/manifest.tsv
ub_overflow_model_cpp/examples/inputs/              小型演示输入
ub_overflow_model_cpp/output/                       可重新生成，禁止提交
...
python3 ub_overflow_model_cpp/scripts/run_corpus_oracle.py <公共参数> \
...
```bash
ub_overflow_model_cpp/output/bin/cvpipeline_ub_model \
  --before-cvpipelining-ir=<input.mlir> \
  --format=json
```
