...
最后核实：2026-07-28，分支 `codex/ub-overflow-model-product`。阶段 0～1 已完成；阶段 2
基础设施/首批 payload 迁移和阶段 3 最低可用 analysis manager 已提交至 `312b3abb4`，但
生产路径仍物化完整 `GenericModule`，因此阶段 2～3 尚未关闭。阶段 0 冻结的优化前模型二进制
来自 `75809a33d`。本文件是 `.agent` 的唯一入口；事实与当前源码或新测量冲突时，应重新核实
