...
usage() {
  cat <<'EOF'
Usage: bash tools/dump_before_cvpipelining_ir.sh [options] [-- extra compiler args...]
...
Options:
  ...
  --run-id NAME
        Dump run name under each adapter. Defaults to compiler_default.
  -c, --compiler FILE
...
extract_cvpipelining_dumps() {
  local tree_dir="$1"
  local case_dir="$2"
  local tsv="$case_dir/cvpipelining_dumps.tsv"
  local cv_file=""
  local before_src=""
  local func_label=""
  local safe_label=""
...
INDEX_ROOT="$OUTPUT_ROOT/index/cvpipelining_${RUN_ID}"
mkdir -p "$ADAPTER_ROOT" "$INDEX_ROOT"

SUMMARY="$INDEX_ROOT/summary.tsv"
printf 'input\tstatus\tcvpipelining_count\tcase_dir\tbefore_snapshots\tafter_snapshots\tstdout\tstderr\n' >"$SUMMARY"
...
for input in "${INPUTS[@]}"; do
  ...
  case_dir="$ADAPTER_ROOT/$adapter/cvpipelining/$RUN_ID"
  ...
  cv_count=$(extract_cvpipelining_dumps "$tree_dir" "$case_dir")

  before_snapshots=""
  after_snapshots=""
  if [[ -f "$case_dir/cvpipelining_dumps.tsv" ]]; then
    before_snapshots=$(awk -F '\t' 'NR > 1 { if (out != "") out = out ","; out = out $3 } END { print out }' "$case_dir/cvpipelining_dumps.tsv")
    after_snapshots=$(awk -F '\t' 'NR > 1 { if (out != "") out = out ","; out = out $4 } END { print out }' "$case_dir/cvpipelining_dumps.tsv")
  fi
