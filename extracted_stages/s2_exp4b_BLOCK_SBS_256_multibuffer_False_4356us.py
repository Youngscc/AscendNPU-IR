...
Optimizations applied:
  ...
  4. BLOCK_SBS=256 + multibuffer=False: Halves SBS iterations (8→4).
     ...
     - Without double-buffering, latency hiding may be reduced but fewer
       domain crossings may compensate
"""
import torch
import triton
...
def triton_sparse_flash_attention_prefill(
    ...
    BLOCK_G = 16
    BLOCK_SBS = 256  # halves SBS iterations (8→4), fewer domain crossings
    BLOCK_K = D_v  # = 512
    ...
    sparse_flash_attention_prefill_kernel_cvpipe[grid](
        ...
        multibuffer=False,  # BLOCK_SBS=256 overflows cbuf with multibuffer=True
