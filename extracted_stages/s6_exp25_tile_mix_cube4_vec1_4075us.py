    ...
    for idx_t1 in range(t1_start, t1_end):
        ...
        for idx_n1_blk in range(0, HEAD_LOOP_TIMES):
            ...
            for idx_sub_sbs in range(0, sbs_loop_times):
                ...
                # Max-based lse (avoids NaN on first iter when lse=-inf):
                # tl.maximum(-inf, x)=x, tl.exp(-inf - x)=0 → new_lse=x. No NaN.
                max_lse = tl.maximum(lse, local_lse)     #[BLOCK_G]
                new_lse = max_lse + tl.log(tl.exp(lse - max_lse) + tl.exp(local_lse - max_lse))     #[BLOCK_G]
                ...
                # SV matmul: BLOCK_V=D_v so single shot (no inner V loop)
                kv_cache = tl.load(Wksp_KNope_ptr + wk_nope_base + offs_sbs[:, None] * stride_wknsbs + tl.arange(0, BLOCK_V)[None, :] * stride_wknd)
