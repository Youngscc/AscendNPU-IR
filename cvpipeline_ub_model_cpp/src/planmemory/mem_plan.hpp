...
inline bool VerifyConflictStage1(
  ...
  if (entry.multiBufferNum > 1 &&
      relations.size() < entry.multiBufferNum)
    return true;

  for (uint32_t i = 0; i < entry.multiBufferNum; ++i) {
    const StorageEntryModel *relation =
        FindStorageEntry(entries, state, relations[i]);
    if (!relation)
      return true;
...
inline void PlanRelationOtherBufferEntryAddress(
  ...
  if (entry.multiBufferNum == 1) {
    ...
    for (size_t i = 0;
         i < entry.otherBufferRelationEntries.size() && i < offsets.size();
         ++i) {
      StorageEntryModel *relation = FindStorageEntry(
          entries, state, entry.otherBufferRelationEntries[i]);
      if (relation)
...
inline PlanMemoryModelResult PlanLocalMemoryImpl(
  ...
  for (uint32_t attempt = firstAttempt; attempt < attemptEnd; ++attempt) {
    ...
    std::map<std::string, uint64_t> buffer2StorageExtentBits;
    std::map<std::string, std::vector<uint64_t>> buffer2Offsets =
        UpdateBuffer2Offsets(entries, planState, &buffer2StorageExtentBits);
    ...
    if (success) {
      ...
      // byte offset plus its original byte-aligned extent.  The 256-bit
      // aligned StorageEntry size controls placement, but is not the reported
      // UB peak extent for a successfully planned buffer.
      result.peakBits = 0;
