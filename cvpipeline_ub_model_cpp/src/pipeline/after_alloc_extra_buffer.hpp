...
inline int AllocationSourceOrder(const BufferAllocation &allocation) {
  if (allocation.ownerOperation >= 0)
    return allocation.ownerOperation;
...
inline AfterAllocExtraBufferState BuildAfterAllocExtraBufferState(PostBufferizationRewriteState postBufferization) {
  ...
  std::vector<Pending> basePending;
  ...
  for (size_t index = 0; index < pending.size(); ++index)
    pending[index].buffer = sourceBuffers[basePending.size() + index];
  ...
  if (preserveCompactedBaseOrder) {
    for (Pending &item : basePending) {
    ...
    pending.insert(pending.begin(),
                   std::make_move_iterator(basePending.begin()),
                   std::make_move_iterator(basePending.end()));
    std::stable_sort(pending.begin(), pending.end(),
                     [](const Pending &lhs, const Pending &rhs) {
                       return std::tie(lhs.operation, lhs.phase,
                                       lhs.ordinal) <
                              std::tie(rhs.operation, rhs.phase,
                                       rhs.ordinal);
                     });
