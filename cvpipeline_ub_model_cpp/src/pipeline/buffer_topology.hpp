...
struct BufferTopology {
  std::vector<BufferTopologyRecord> localBuffers;
};
...
public:
  ...
  BufferTopology Build() {
    BufferTopology topology;
...
inline BufferTopology
BuildBufferTopology(const GenericModule &module) {
  return BufferTopologyBuilder(module).Build();
}

inline std::string SerializeBufferTopology(
    const BufferTopology &topology) {
  std::ostringstream output;
  output << "BUFFER_TOPOLOGY\t1\n";
  for (const BufferTopologyRecord &buffer : topology.localBuffers) {
    ...
    for (const BufferUse &use : buffer.uses)
