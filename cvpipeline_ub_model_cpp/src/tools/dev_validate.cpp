...
#include "../passes/cvpipelining/cvpipelining.hpp"
#include "../passes/cvpipelining/cvpipelining_analysis.hpp"
#include "../passes/cvpipelining/cvpipelining_pass.hpp"
#include "../passes/cvpipelining/cvpipelining_preload_rewrite.hpp"
#include "../passes/cvpipelining/cvpipelining_rewrite.hpp"
...
#include "../pipeline/after_mark_multi_buffer.hpp"
#include "../pipeline/plan_memory_input_semantic_ir.hpp"
#include "../pipeline/plan_memory_input_builder.hpp"
...
#include "../validation/plan_memory_input_semantic_ir_oracle.hpp"
...
Options parseOptions(int argc, char **argv) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (auto actionValue = readValue("--action"))
      ...
      opts.enableCodeMotion = *codeMotion == "1" || *codeMotion == "true";
      ...
      std::cout << "Usage: cvpipeline_ub_model_dev_validate "
                   ...
                   "validate-plan-memory-attempts|validate-memory-plan|"
                   "validate-fixed-seed-lifetimes|"
                   "validate-fixed-seed-plans|dump-after-cvpipelining-semantic-ir|"
                   "dump-generic-semantic-ir|"
                   "model-loop-invariant-code-motion|"
                   "model-loop-invariant-code-motion-after-canonicalization-hivm|"
                   "model-canonicalization-hivm-only|"
                   "model-tile-and-bind-sub-block-attempt|"
                   "model-tile-and-bind-sub-block-after-first-hoist|"
                   "model-tile-and-bind-sub-block-after-bubble-cse|"
                   "model-tile-and-bind-sub-block-after-arith-to-affine|"
                   "model-tile-and-bind-sub-block-affine-canonicalization-state|"
                   "model-mark-real-core-type|"
                   "dump-cross-core-sync-dependencies|"
                   "model-cross-core-gss-only|"
                   "dump-buffer-topology|dump-one-shot-bufferize-allocation-oracle|"
                   "dump-ub-function-one-shot-bufferize-allocation-oracle|"
                   "model-one-shot-bufferize-allocations|"
                   "model-one-shot-bufferize-allocations|"
                   "model-bufferized-semantic-ir|"
                   "validate-bufferized-semantic-ir|"
                   "validate-one-shot-bufferize-semantic-ir|"
                   "dump-one-shot-analysis-oracle|"
                   "dump-ub-function-one-shot-analysis-oracle|"
                   "model-one-shot-analysis|"
                   "model-one-shot-analysis|"
                   ...
                   "model-hivm-decompose-operation-delta|model-hivm-decompose-op-semantic-ir|"
                   "model-mark-multi-buffer-projection|"
                   "model-mark-multi-buffer-state|"
                   "model-plan-memory-input-buffers-from-before-one-shot-bufferize|"
                   "model-plan-memory-input-access-debug-from-before-one-shot-bufferize> "
...
int validatePlanMemoryAttempts(const Options &opts) {
  ...
  for (const cvub::fs::path &caseDir : listCaseDirs(opts.root)) {
    ...
    cvub::fs::path dump = cvub::fs::path(opts.attemptRoot) /
                          caseDir.filename() / "plan_memory_attempts.tsv";
...
int main(int argc, char **argv) {
  ...
  try {
    ...
    if (opts.action == "validate-plan-memory-attempts")
    ...
    if (opts.action == "dump-dimension-analysis") {
      ...
      module = cvub::RunGlobalWorkspacePlan(std::move(module));
      module = cvub::RunCanonicalizationHIVMPipeline(std::move(module));
      module = cvub::RunMarkRealCoreType(std::move(module));
      module = cvub::RunCrossCoreGSS(std::move(module));
      module = cvub::RunMarkRealCoreType(std::move(module), true);
      module = cvub::RunSplitMixKernel(std::move(module));
      for (const cvub::GenericOperation &function : module.operations) {
        if (!cvub::IsMixAIVFunction(function))
          continue;
        cvub::DimensionAnalyzer analyzer(module);
        analyzer.initialize();
        analyzer.computeTilingDim(function);
        writeTextFile(opts.output, analyzer.serializeAnalysis());
        return 0;
      }
      throw std::runtime_error(
          "dump-dimension-analysis: no mix AIV function found");
    }
    if (opts.action == "diagnose-tile-and-bind-sub-block") {
      if (opts.root.empty() || opts.output.empty()) {
        std::cerr << "[ERROR] --root and --output are required\n";
        return 1;
      }
      ...
      module = cvub::RunMarkRealCoreType(std::move(module), true);
      module = cvub::RunSplitMixKernel(std::move(module));
      module = cvub::RunInlineScope(std::move(module));
      std::ostringstream report;
      report << "HAS_BATCH_MATMUL_LOOP\t"
             << cvub::HasBatchMatmulLoopInAICFunctions(module) << "\n"
             << "HAS_IMPLICIT_TRANSPOSE_LAST_AXIS\t"
             << cvub::HasImplicitTransposeWithLastAxisInAIVFunctions(module)
             << "\n"
             << "LOAD_STORE_SAME_ADDRESS\t"
             << cvub::AreLoadAndStoreSameAddress(module) << "\n";
      for (const cvub::GenericOperation &function : module.operations) {
        if (!cvub::IsMixAIVFunction(function))
          continue;
        cvub::GenericModule candidate = module;
        cvub::DimensionAnalyzer analyzer(candidate);
        const bool initialized = analyzer.initialize();
        if (initialized)
          analyzer.computeTilingDim(
              candidate.operations.at(static_cast<size_t>(function.id)));
        const bool selected = initialized && analyzer.hasSelectedTilingDim();
        const auto definitions = cvub::DefiningOperations(candidate);
        const auto valueTypes = cvub::ValueTypes(candidate);
        std::set<int> tiledStartPoints;
        for (int operationId : cvub::GetTileAndBindDescendants(
                 candidate,
                 candidate.operations.at(static_cast<size_t>(function.id)))) {
          const cvub::GenericOperation &operation =
              candidate.operations.at(static_cast<size_t>(operationId));
          if (selected && cvub::IsTileAndBindStoreCopyStartPoint(
                              candidate, operation, analyzer, valueTypes))
            tiledStartPoints.insert(operationId);
        }
        std::vector<std::tuple<int, int, bool>> startPointChecks;
        for (int operationId : tiledStartPoints) {
          const cvub::GenericOperation &operation =
              candidate.operations.at(static_cast<size_t>(operationId));
          int source = operation.operands.front();
          int consumer = operation.id;
          bool checkCurrentUsers = true;
          auto sourceDefinition = definitions.find(source);
          auto sourceType = valueTypes.find(source);
          const auto shaped = sourceType == valueTypes.end()
                                  ? std::nullopt
                                  : cvub::ParseShapedTypeForDimensionAnalysis(
                                        sourceType->second);
          if (shaped &&
              std::any_of(shaped->shape.begin(), shaped->shape.end(),
            ...
            source = sourceDefinition->second->operands.front();
            consumer = sourceDefinition->second->id;
            checkCurrentUsers = false;
          }
          std::set<int> visited;
          const bool canBubble = cvub::CanBubbleUpTileAndBindValue(
              candidate, source,
              static_cast<size_t>(
                  analyzer.getTilingDim(operation.operands.front())),
              analyzer, definitions, valueTypes, consumer, tiledStartPoints,
              checkCurrentUsers, visited);
          startPointChecks.emplace_back(operation.id, source, canBubble);
        }
        const bool bubble =
            selected && cvub::VerifyMarkedExtractSlicesCanBubbleUp(
                            candidate,
                            candidate.operations.at(
                                static_cast<size_t>(function.id)),
                            analyzer);
        const bool attempted =
            bubble && cvub::AttemptBindSubBlock(candidate, function.id);
        const bool reshapes =
            attempted && cvub::VerifyTileAndBindReshapeTypes(candidate);
        report << "FUNCTION\t" << function.id << "\n"
               << "ANALYZER_INITIALIZED\t" << initialized << "\n"
               << "TILING_DIM_SELECTED\t" << selected << "\n"
               << "BUBBLE_PRECONDITIONS\t" << bubble << "\n"
               << "ATTEMPT_COMPLETED\t" << attempted << "\n"
               << "RESHAPE_TYPES_VALID\t" << reshapes << "\n";
        for (const auto &[operationId, source, canBubble] : startPointChecks)
          report << "START_POINT\t" << operationId << '\t' << source << '\t'
                 << canBubble << "\n";
      }
      writeTextFile(opts.output, report.str());
      return 0;
    }
    if (opts.action == "model-global-workspace-plan" ||
        opts.action == "model-mark-real-core-type" ||
        opts.action == "dump-cross-core-sync-dependencies" ||
        opts.action == "model-cross-core-gss" ||
        opts.action == "model-split-mix-kernel" ||
        opts.action == "model-tile-and-bind-sub-block-attempt" ||
        opts.action == "model-tile-and-bind-sub-block-after-first-hoist" ||
        opts.action == "model-tile-and-bind-sub-block-after-bubble-cse" ||
        opts.action == "model-tile-and-bind-sub-block-before-canonicalization" ||
        opts.action ==
            "model-tile-and-bind-sub-block-after-arith-to-affine" ||
        opts.action ==
            "model-tile-and-bind-sub-block-affine-canonicalization-state" ||
        opts.action == "model-tile-and-bind-sub-block" ||
        opts.action == "model-one-shot-bufferize-input-pipeline" ||
        opts.action == "model-pre-bufferization-ub-function") {
      if (opts.root.empty() || opts.output.empty()) {
        std::cerr << "[ERROR] --root and --output are required\n";
        return 1;
      }
      cvub::GenericModule module = cvub::ParseGenericIR(opts.root, false);
      cvub::ApplyOperationSemanticsToAll(module.operations);
      if (opts.action == "model-one-shot-bufferize-input-pipeline" ||
          opts.action == "model-pre-bufferization-ub-function") {
        cvub::UBAffectingPassOptions passOptions;
        passOptions.enableCodeMotion = opts.enableCodeMotion;
        module = cvub::RunPassesBeforeOneShotBufferize(std::move(module),
                                                       passOptions);
        if (opts.action == "model-pre-bufferization-ub-function")
          module = cvub::ProjectUBFunctionForOracle(std::move(module));
        writeTextFile(opts.output, cvub::SerializeGenericModule(module));
        return 0;
      }
      ...
      module = cvub::RunGlobalWorkspacePlan(std::move(module));
      module = cvub::RunCanonicalizationHIVMPipeline(std::move(module));
      if (opts.action == "model-mark-real-core-type") {
        module = cvub::RunMarkRealCoreType(std::move(module));
        writeTextFile(opts.output, cvub::SerializeGenericModule(module));
        return 0;
      }
      if (opts.action == "dump-cross-core-sync-dependencies") {
        module = cvub::RunMarkRealCoreType(std::move(module));
        std::ostringstream report;
        for (const cvub::GenericOperation &operation : module.operations) {
          if (!cvub::IsSplitMixFunction(operation))
            continue;
          for (int operationId :
               cvub::GetFunctionDescendants(module, operation)) {
            const cvub::GenericOperation &descendant =
                module.operations.at(static_cast<size_t>(operationId));
            const cvub::SplitMixCoreType core =
                cvub::GetSplitMixCoreType(descendant);
            if (core == cvub::SplitMixCoreType::Common)
              continue;
            report << "CORE\t" << operationId << '\t' << descendant.name
                   << '\t';
            if (core == cvub::SplitMixCoreType::Both)
              report << "CUBE_AND_VECTOR";
            else
              report << cvub::SplitMixCoreTypeName(core);
            report << '\n';
          }
          for (const cvub::CrossCoreSyncDependency &dependency :
               cvub::FindCrossCoreSyncDependencies(module, operation)) {
            report << "DEPENDENCY\t" << dependency.producer << '\t'
                   << dependency.consumer << '\t'
                   << cvub::SplitMixCoreTypeName(dependency.producerCore)
                   << '\t'
                   << cvub::SplitMixCoreTypeName(dependency.consumerCore)
                   << '\n';
          }
        }
        writeTextFile(opts.output, report.str());
      ...
      if (opts.action == "model-split-mix-kernel" ||
          opts.action == "model-tile-and-bind-sub-block-attempt" ||
          opts.action == "model-tile-and-bind-sub-block-after-first-hoist" ||
          opts.action == "model-tile-and-bind-sub-block-after-bubble-cse" ||
          opts.action == "model-tile-and-bind-sub-block-before-canonicalization" ||
      ...
      if (opts.action == "model-tile-and-bind-sub-block-attempt" ||
          opts.action == "model-tile-and-bind-sub-block-after-first-hoist" ||
          opts.action == "model-tile-and-bind-sub-block-after-bubble-cse") {
        module = cvub::RunTileAndBindSubBlockEarlyPatterns(std::move(module));
        std::vector<int> functions;
        for (const cvub::GenericOperation &operation : module.operations)
          if (cvub::IsMixAIVFunction(operation))
            functions.push_back(operation.id);
        for (int functionId : functions)
          if (!cvub::AttemptBindSubBlock(module, functionId)) {
            std::cerr << "[ERROR] TileAndBindSubBlock attempt failed\n";
            return 1;
          }
        if (opts.action != "model-tile-and-bind-sub-block-attempt")
          for (int functionId : functions)
            cvub::RunTileAndBindHoistAffine(module, functionId);
        if (opts.action == "model-tile-and-bind-sub-block-after-bubble-cse")
          for (int functionId : functions)
            cvub::RunTileAndBindBubbleUpCSE(module, functionId);
        cvub::ApplyOperationSemanticsToAll(module.operations);
        writeTextFile(opts.output, cvub::SerializeGenericModule(module));
        return 0;
      }
      if (opts.action == "model-tile-and-bind-sub-block-before-canonicalization" ||
          opts.action ==
              "model-tile-and-bind-sub-block-after-arith-to-affine" ||
          opts.action ==
              "model-tile-and-bind-sub-block-affine-canonicalization-state" ||
        ...
        if (opts.action ==
            "model-tile-and-bind-sub-block-affine-canonicalization-state") {
          cvub::ConvertArithToAffineState state =
              cvub::RunConvertArithToAffine(module, false);
          cvub::RunAffineApplyCanonicalization(module, state);
          cvub::RunAffineMinMaxCanonicalization(module, state);
          cvub::RunAffineApplyCompositionForMinMax(module, state);
          cvub::RunAffineMinMaxCanonicalization(module, state);
          cvub::RunAffineCanonicalizationCSE(module, state);
          writeTextFile(opts.output,
                        cvub::SerializeConvertArithToAffineState(module,
                                                                 state));
          return 0;
        }
        if (opts.action ==
        ...
        else if (opts.action == "model-tile-and-bind-sub-block")
          module = cvub::RunCanonicalizationHIVMPipelineSourceAligned(
              std::move(module));
      }
      cvub::ApplyOperationSemanticsToAll(module.operations);
      writeTextFile(opts.output, cvub::SerializeGenericModule(module));
    ...
    if (opts.action == "model-loop-invariant-code-motion") {
      if (opts.root.empty() || opts.output.empty()) {
        std::cerr << "[ERROR] --root and --output are required\n";
        return 1;
      }
      ...
      if (opts.enableCodeMotion)
        cvub::RunLoopInvariantCodeMotion(module);
    ...
    if (opts.action ==
      ...
      cvub::GenericModule module =
          cvub::RunPassesBeforeLoopInvariantCodeMotion(
    ...
    if (opts.action == "model-canonicalization-hivm-only" ||
        opts.action == "model-cross-core-gss-only") {
      if (opts.root.empty() || opts.output.empty()) {
        std::cerr << "[ERROR] --root and --output are required\n";
        return 1;
      }
      cvub::GenericModule module = cvub::ParseGenericIR(opts.root, false);
      cvub::ApplyOperationSemanticsToAll(module.operations);
      module = opts.action == "model-canonicalization-hivm-only"
                   ? cvub::RunCanonicalizationHIVMPipeline(std::move(module))
                   : cvub::RunCrossCoreGSS(std::move(module));
      cvub::ApplyOperationSemanticsToAll(module.operations);
      writeTextFile(opts.output, cvub::SerializeGenericModule(module));
      return 0;
    }
    ...
    if (opts.action == "dump-plan-memory-input-buffer-oracle") {
      if (opts.root.empty() || opts.output.empty()) {
        std::cerr << "[ERROR] --root and --output are required\n";
        return 1;
      }
    ...
    if (opts.action == "dump-plan-memory-input-access-oracle") {
      if (opts.root.empty() || opts.output.empty()) {
        std::cerr << "[ERROR] --root and --output are required\n";
        return 1;
      }
    ...
    if (opts.action == "dump-plan-memory-operation-sequence-oracle") {
      if (opts.root.empty() || opts.output.empty()) {
        std::cerr << "[ERROR] --root and --output are required\n";
        return 1;
      }
    ...
    if (opts.action == "dump-plan-memory-structured-input-oracle") {
      if (opts.root.empty() || opts.output.empty()) {
        std::cerr << "[ERROR] --root and --output are required\n";
        return 1;
      }
    ...
    if (opts.action == "dump-plan-memory-canonical-input-oracle") {
      if (opts.root.empty() || opts.output.empty()) {
        std::cerr << "[ERROR] --root and --output are required\n";
        return 1;
      }
    ...
    if (opts.action == "dump-buffer-topology") {
      if (opts.root.empty() || opts.output.empty()) {
        std::cerr << "[ERROR] --root and --output are required\n";
        return 1;
      }
      const cvub::GenericModule module =
          cvub::ParseGenericIR(opts.root, false);
      writeTextFile(opts.output, cvub::SerializeBufferTopology(
                                      cvub::BuildBufferTopology(module)));
      return 0;
    }
    if (opts.action == "model-convert-arith-to-affine" ||
        opts.action == "model-convert-arith-to-affine") {
      if (opts.root.empty() || opts.output.empty()) {
        std::cerr << "[ERROR] --root and --output are required\n";
        return 1;
      }
      ...
      if (opts.action == "model-convert-arith-to-affine") {
        cvub::UBAffectingPassOptions passOptions;
        passOptions.enableCodeMotion = opts.enableCodeMotion;
        module = cvub::RunPassesBeforeOneShotBufferize(
            std::move(module), passOptions);
      } else if (opts.enableCodeMotion) {
        cvub::RunLoopInvariantCodeMotion(module);
      }
      cvub::ConvertArithToAffineState state =
          cvub::RunConvertArithToAffine(module);
      cvub::RunAffineMinMaxCanonicalization(module, state);
      cvub::RunAffineCanonicalizationCSE(module, state);
      writeTextFile(opts.output,
                    cvub::SerializeConvertArithToAffineState(module, state));
      return 0;
    }
    if (opts.action == "dump-one-shot-bufferize-allocation-oracle" ||
        opts.action == "dump-ub-function-one-shot-bufferize-allocation-oracle" ||
      ...
      cvub::GenericModule module = cvub::ParseGenericIR(opts.root, false);
      if (opts.action == "dump-ub-function-one-shot-bufferize-allocation-oracle")
        module = cvub::ProjectUBFunctionForOracle(std::move(module));
      if (opts.action == "model-one-shot-bufferize-allocations") {
        cvub::UBAffectingPassOptions passOptions;
        passOptions.enableCodeMotion = opts.enableCodeMotion;
        module = cvub::RunPassesBeforeOneShotBufferize(
            std::move(module), passOptions);
      ...
      const std::vector<cvub::BufferAllocation> allocations =
          (opts.action == "dump-one-shot-bufferize-allocation-oracle" ||
           opts.action == "dump-ub-function-one-shot-bufferize-allocation-oracle")
      ...
      writeTextFile(
          opts.output,
          opts.action == "model-one-shot-bufferize-allocations"
              ? cvub::SerializeBufferAllocationDetails(allocations)
    ...
    if (opts.action == "model-hivm-decompose-op-semantic-ir") {
      if (opts.root.empty() || opts.output.empty()) {
        std::cerr << "[ERROR] --root and --output are required\n";
        return 1;
      }
    ...
    if (opts.action == "model-alloc-extra-buffer-semantic-ir" ||
        opts.action == "model-alloc-extra-buffer-buffers") {
      if (opts.root.empty() || opts.output.empty()) {
        std::cerr << "[ERROR] --root and --output are required\n";
        return 1;
      }
    ...
    if (opts.action == "model-alloc-extra-buffer-projection" ||
        opts.action == "dump-alloc-extra-buffer-projection-oracle" ||
        opts.action == "dump-alloc-extra-buffer-buffers-oracle") {
      if (opts.root.empty() || opts.output.empty() ||
          (opts.action != "model-alloc-extra-buffer-projection" &&
    ...
    if (opts.action == "model-inline-load-copy-semantic-ir" ||
        opts.action == "model-inline-load-copy-buffer-projection") {
      if (opts.root.empty() || opts.output.empty()) {
        std::cerr << "[ERROR] --root and --output are required\n";
        return 1;
      }
    ...
    if (opts.action == "model-mark-multi-buffer-state" ||
        opts.action == "model-mark-multi-buffer-projection") {
      if (opts.root.empty() || opts.output.empty()) {
        std::cerr << "[ERROR] --root and --output are required\n";
        return 1;
      }
    ...
    if (opts.action ==
      ...
      const cvub::PlanMemoryInputSemanticIR planMemoryInputIR =
          cvub::BuildPlanMemoryInputSemanticIR(
              cvub::BuildAfterMarkMultiBufferState(
                  cvub::BuildAfterInlineLoadCopyState(
                      cvub::BuildAfterAllocExtraBufferState(
                          cvub::BuildPostBufferizationRewriteState(
                              cvub::BuildBufferizedSemanticIR(
                                  module, bufferization)))),
                  options));
      writeTextFile(
          opts.output,
    ...
    if (opts.action == "model-plan-memory-input-semantic-ir" ||
        opts.action == "model-plan-memory-input-buffers" ||
        opts.action == "model-plan-memory-input-accesses" ||
        opts.action == "model-plan-memory-input-access-debug" ||
        opts.action == "model-plan-memory-structured-input" ||
        opts.action == "model-plan-memory-canonical-input" ||
        opts.action == "model-plan-memory-operation-sequence" ||
        opts.action == "model-plan-memory-structured-lifetime" ||
        opts.action == "model-plan-memory-live-shuffle" ||
        opts.action == "model-plan-memory-structured-plan") {
      if (opts.root.empty() || opts.output.empty()) {
        std::cerr << "[ERROR] --root and --output are required\n";
        return 1;
      }
          ...
          cvub::BuildPlanMemoryInputSemanticIR(cvub::BuildAfterMarkMultiBufferState(
              cvub::BuildAfterInlineLoadCopyState(cvub::BuildAfterAllocExtraBufferState(
                  cvub::BuildPostBufferizationRewriteState(cvub::BuildBufferizedSemanticIR(
                      module, bufferization)))),
              options));
      if (opts.action == "model-plan-memory-structured-input" ||
          opts.action == "model-plan-memory-canonical-input" ||
          opts.action == "model-plan-memory-operation-sequence" ||
          opts.action == "model-plan-memory-structured-lifetime" ||
          opts.action == "model-plan-memory-live-shuffle" ||
        ...
        if (opts.action == "model-plan-memory-canonical-input") {
          writeTextFile(opts.output,
                        cvub::SerializeCanonicalPlanMemoryInput(input));
          return 0;
        }
        if (opts.action == "model-plan-memory-operation-sequence") {
          writeTextFile(opts.output,
        ...
        if (opts.action == "model-plan-memory-live-shuffle") {
        ...
        if (opts.action == "model-plan-memory-structured-lifetime") {
          const cvub::LifetimeAnalysis analysis =
              cvub::analyzeLifetimes(input, seed, false);
    ...
    if (opts.action == "validate-hivm-decompose-op-rewrites") {
      if (opts.root.empty() || opts.attemptRoot.empty() ||
          opts.afterIr.empty() || opts.output.empty() ||
          opts.function.empty()) {
        std::cerr << "[ERROR] HIVMDecomposeOp rewrite validation requires source --root, "
      ...
      const cvub::PostBufferizationRewriteValidation validation =
          cvub::ValidatePostBufferizationOperationRewrites(model, before, after,
                                            opts.function);
      std::ostringstream report;
      report << "HIVM_DECOMPOSE_OP_REWRITE_VALIDATION\t1\nSUMMARY\t"
    ...
    if (opts.action == "validate-one-shot-bufferize-semantic-ir") {
      if (opts.root.empty() || opts.attemptRoot.empty() ||
          opts.afterIr.empty() || opts.output.empty()) {
        std::cerr << "[ERROR] --root, --attempt-root, --after-ir and "
                     "--output are required\n";
        return 1;
      }
      cvub::GenericModule module = cvub::ParseGenericIR(opts.root, false);
      ...
      cvub::GenericModule oracleBefore =
          cvub::ParseGenericIR(opts.attemptRoot, false);
      cvub::ApplyOperationSemanticsToAll(oracleBefore.operations);
      oracleBefore =
          cvub::ProjectUBFunctionForOracle(std::move(oracleBefore));
      const bool prebufferExact =
          cvub::SerializeGenericModule(module) ==
          cvub::SerializeGenericModule(oracleBefore);

      const cvub::OneShotBufferizationResult bufferization =
          cvub::RunOneShotBufferize(module);
      const cvub::BufferizedSemanticIR model =
          cvub::BuildBufferizedSemanticIR(module, bufferization);
      const cvub::GenericModule oracleAfter =
          cvub::ProjectUBFunctionForOracle(
              cvub::ParseGenericIR(opts.afterIr, false));
      cvub::BufferizedSemanticIRValidation validation =
          cvub::ValidateBufferizedSemanticIR(module, model, oracleAfter);
      const bool operationCountsExact = std::none_of(
          validation.errors.begin(), validation.errors.end(),
          [](const std::string &error) {
            return cvub::startsWith(error, "operation count mismatch");
          });

      const std::vector<cvub::BufferAllocation> oracleAllocationRecords =
          cvub::CollectBufferAllocationOracle(oracleAfter);
      const std::string modelAllocations =
          cvub::SerializeBufferAllocations(model.allocations);
      const std::string oracleAllocations =
          cvub::SerializeBufferAllocations(oracleAllocationRecords);
      const bool allocationsExact = modelAllocations == oracleAllocations;
      if (!allocationsExact) {
        validation.errors.push_back(
            "OneShotBufferize allocation projection is not byte exact");
        const size_t common =
            std::min(model.allocations.size(), oracleAllocationRecords.size());
        for (size_t index = 0; index < common; ++index) {
          const cvub::BufferAllocation &modeled = model.allocations[index];
          const cvub::BufferAllocation &oracle = oracleAllocationRecords[index];
          if (modeled.type == oracle.type &&
              modeled.alignment == oracle.alignment &&
              modeled.dynamicExtentCount == oracle.dynamicExtentCount)
            continue;
          validation.errors.push_back(
              "allocation " + std::to_string(index) + ": " + modeled.type +
              " alignment=" + modeled.alignment + " dynamic=" +
              std::to_string(modeled.dynamicExtentCount) + " != " +
              oracle.type + " alignment=" + oracle.alignment + " dynamic=" +
              std::to_string(oracle.dynamicExtentCount));
          break;
        }
      }

      std::ostringstream report;
      report << "ONE_SHOT_BUFFERIZE_SEMANTIC_IR_VALIDATION\t1\n"
             << "PREBUFFER_TEXT_EXACT\t" << (prebufferExact ? 1 : 0) << '\n'
             << "OPERATION_COUNTS_EXACT\t"
             << (operationCountsExact ? 1 : 0) << '\n'
             << "ALLOCATIONS_EXACT\t" << (allocationsExact ? 1 : 0)
             << '\n' << "MODEL_ALLOCATIONS\t" << model.allocations.size()
             << '\n' << "ORACLE_ALLOCATIONS\t"
             << oracleAllocationRecords.size() << '\n'
             << "SUMMARY\t" << validation.checkedValues << '\t'
             << validation.checkedAccesses << '\t'
             << validation.errors.size() << '\n';
      for (const std::string &error : validation.errors)
    ...
    if (opts.action == "dump-one-shot-analysis-oracle" ||
        opts.action == "dump-ub-function-one-shot-analysis-oracle" ||
      ...
      if (opts.action == "dump-ub-function-one-shot-analysis-oracle")
        module = cvub::ProjectUBFunctionForOracle(std::move(module));
      if (opts.action == "model-one-shot-analysis")
        for (cvub::GenericOperation &operation : module.operations)
          if (cvub::HasModeledOperationSemantics(operation.name))
