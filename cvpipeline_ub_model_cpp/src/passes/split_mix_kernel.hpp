...
inline bool IsSplitMixTriviallyDead(const GenericModule &module,
                                    ...
                                    const std::map<int, size_t> &users) {
  ...
  if (std::any_of(operation.results.begin(), operation.results.end(),
                  [&](int result) {
                    auto count = users.find(result);
                    return count != users.end() && count->second != 0;
                  }))
    return false;
...
inline void PostProcessSplitMixCubeFunc(
  ...
  for (int operationId : descendants) {
    ...
    if (replacementMark && operation.operands.size() >= 2) {
      ReplaceSplitMixValue(module, operation.operands[0],
...
inline GenericModule RunSplitMixKernelProjection(
    ...
    for (int operationId : descendants) {
      ...
      if (operation.name == "scf.for" && operation.operands.size() >= 2) {
        ...
        for (size_t index = 0; index < operation.results.size(); ++index)
          ReplaceSplitMixValue(module, operation.results[index],
  ...
  for (int functionId : vectorFunctions) {
  ...
  for (int functionId : vectorFunctions)
    RunSplitMixOperationFolder(module, functionId, activeOperations);
  while (FoldSplitMixBooleanOperation(module, activeOperations)) {
  }
  ...
  while (erased) {
    erased = false;
    std::map<int, size_t> users;
    for (int operationId : activeOperations)
      for (int operand :
           module.operations.at(static_cast<size_t>(operationId)).operands)
        ++users[operand];
    for (auto iterator = activeOperations.rbegin();
         iterator != activeOperations.rend(); ++iterator) {
      const int operationId = *iterator;
      const GenericOperation &operation =
          module.operations.at(static_cast<size_t>(operationId));
