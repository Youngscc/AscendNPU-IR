...
public:
  ...
  std::vector<int> getMemoryOps(const std::vector<int> &values) const {
    std::vector<int> roots;
    std::set<int> seen;
    for (int value : values)
      for (int root : tracebackMemVals(value))
...
GetCrossCoreReadWriteMemoryOps(const GenericOperation &operation,
                               const CrossCoreMemoryValueTracer &tracer) {
  std::vector<int> reads;
  std::vector<int> writes;
  if (IsDestinationStyleOp(operation.name)) {
...
FindCrossCoreSyncDependencies(const GenericModule &module,
  ...
  for (int operationId : descendants) {
    const GenericOperation &operation =
        module.operations.at(static_cast<size_t>(operationId));
    if (std::optional<CrossCoreMemoryAccess> access =
            GetCrossCoreReadWriteMemoryOps(operation, tracer))
      accesses.emplace(operationId, std::move(*access));
  }
  ...
  for (size_t consumerIndex = 0; consumerIndex < descendants.size();
    ...
    const auto consumerAccess = accesses.find(consumerId);
    if (consumerAccess == accesses.end())
      continue;
    ...
    if (consumerCore == SplitMixCoreType::Common)
      continue;
    for (size_t producerIndex = consumerIndex; producerIndex-- > 0;) {
      const int producerId = descendants[producerIndex];
      const auto producerAccess = accesses.find(producerId);
      if (producerAccess == accesses.end())
        continue;
      ...
      if (producerCore == SplitMixCoreType::Common ||
          producerCore == SplitMixCoreType::Both)
        continue;
      const SplitMixCoreType resolvedConsumerCore =
          consumerCore == SplitMixCoreType::Both
              ...
              : consumerCore;
      if (producerCore == resolvedConsumerCore ||
          !CrossCoreMemoryHazard(producerAccess->second,
                                 consumerAccess->second))
        continue;
...
inline std::vector<int> CrossCoreAncestorLoops(const GenericModule &module,
                                               int operationId) {
  std::vector<int> result;
...
inline std::optional<int> OutermostCrossCoreCommonLoop(
    const GenericModule &module, int producer, int consumer) {
  const std::vector<int> producerLoops =
      CrossCoreAncestorLoops(module, producer);
  const std::vector<int> consumerLoops =
      CrossCoreAncestorLoops(module, consumer);
  for (int loop : producerLoops)
    if (std::find(consumerLoops.begin(), consumerLoops.end(), loop) !=
        consumerLoops.end())
      return loop;
  return std::nullopt;
}

inline std::optional<int> InnermostCrossCoreCommonLoop(
    const GenericModule &module, int producer, int consumer) {
  const std::vector<int> producerLoops =
      CrossCoreAncestorLoops(module, producer);
  const std::vector<int> consumerLoops =
      CrossCoreAncestorLoops(module, consumer);
  for (auto loop = producerLoops.rbegin(); loop != producerLoops.rend(); ++loop)
    if (std::find(consumerLoops.begin(), consumerLoops.end(), *loop) !=
        consumerLoops.end())
      return *loop;
  return std::nullopt;
...
inline GenericModule RunCrossCoreGSS(GenericModule module) {
  ...
  for (int functionId : mixFunctions) {
    ...
    for (size_t index = 0; index < dependencies.size(); ++index) {
      ...
      const std::optional<int> innermostLoop =
          InnermostCrossCoreCommonLoop(module, dependency.producer,
                                       dependency.consumer);
      const std::optional<int> outermostLoop =
          OutermostCrossCoreCommonLoop(module, dependency.producer,
                                       dependency.consumer);
      if (innermostLoop && outermostLoop)
