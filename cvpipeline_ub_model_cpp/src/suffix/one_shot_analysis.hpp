...
ModelOneShotBufferizeAllocationsExact(
  ...
  auto tensorEmptyAllocationPosition =
      [&](const GenericOperation &operation, int resultValue) {
        auto use = soleUses.find(resultValue);
        if (use == soleUses.end())
          return std::pair<int, int>{operation.id, 0};
        const GenericOperation &user =
            module.operations.at(static_cast<size_t>(use->second.first));
        const std::vector<size_t> initOperands = DpsInitOperandIndices(
            user.name, user.operands.size(), user.properties);
        if (std::find(initOperands.begin(), initOperands.end(),
                      use->second.second) == initOperands.end())
          return std::pair<int, int>{operation.id, 0};
        return std::pair<int, int>{
            user.id, -1000000 + static_cast<int>(use->second.second)};
      };
    ...
    if (operation.name == "memref.alloc") {
      ...
      for (size_t index = 0; index < operation.resultTypes.size(); ++index) {
        ...
        if (!IsTensorType(type))
          continue;
        std::pair<int, int> position = {operation.id, static_cast<int>(index)};
        if (operation.name == "tensor.empty" && index < operation.results.size())
          position = tensorEmptyAllocationPosition(operation,
