  ...
  for (const GenericOperation &operation : module.operations) {
    for (size_t operand = 0; operand < operation.operandTypes.size(); ++operand) {
      ...
      if (definition != definitions.end() &&
          ...
          useCounts[operation.operands[operand]] == 1)
        continue;
      ...
      result.push_back({ConvertTensorToMemRefType(tensorType), "64 : i64",
                        CountDynamicTensorExtents(tensorType),
                        allocationSource, dynamicExtentValues});
    }
    if (operation.name == "memref.alloc") {
        ...
        if (IsMemRefType(type))
          result.push_back({type,
                            ...
                            "result:" + std::to_string(operation.id) + ":" +
                                std::to_string(index), operation.operands});
      }
      ...
      for (size_t index = 0; index < operation.resultTypes.size(); ++index) {
        ...
        if (IsTensorType(type))
          result.push_back({ConvertTensorToMemRefType(type), "64 : i64",
...
RunOneShotBufferize(const GenericModule &module) {
  ...
  result.allocations =
      ModelOneShotBufferizeAllocationsExact(module, result.decisions);
  return result;
}
