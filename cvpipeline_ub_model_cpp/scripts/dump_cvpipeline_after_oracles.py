#!/usr/bin/env python3
"""Generate exact CVPipelining-after IR oracles with provenance."""
...
def main() -> int:
    ...
    if not inputs or not configs:
        raise SystemExit("[ERROR] C0 requires at least one input and one config")

    adapter_root = output_root / "adapters"
    index_root = output_root / "index/c0_cvpipeline_after"
    ...
    for input_path in inputs:
        ...
        for config in configs:
            ...
            run_dir = (adapter_root / adapter / "01_cvpipeline/c0_oracle" /
                       snapshot / config_id)
            ...
            summary_rows.append({
                ...
                "input": input_path,
                "after_cvpipeline_ir": after_ir,
                "boundary_dump": boundary_dump,
                "config_json": config_file,
                "command_json": command_file,
                "input_sha256": input_hash,
                "after_cvpipeline_sha256": sha256_file(after_ir) if after_ir.is_file() else "",
