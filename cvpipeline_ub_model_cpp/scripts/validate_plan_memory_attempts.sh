...
ATTEMPT_ROOT="${PLANMEMORY_ATTEMPT_ROOT:-${MODULE_DIR}/output/plan_memory_attempt_oracle}"

mkdir -p "${REPORT_DIR}"
"${VALIDATOR}" \
  --action=validate-plan-memory-attempts \
  --root="${SUFFIX_ROOT}" \
  --attempt-root="${ATTEMPT_ROOT}" \
  --output="${REPORT_DIR}/plan_memory_attempt_validation.tsv"
