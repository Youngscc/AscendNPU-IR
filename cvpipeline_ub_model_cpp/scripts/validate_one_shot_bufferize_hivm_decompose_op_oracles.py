#!/usr/bin/env python3
"""Validate OneShotBufferize and HIVMDecomposeOp native snapshots."""
...
from dump_one_shot_bufferize_hivm_decompose_op_oracles import snapshots_complete, tree_sha256
...
def main() -> int:
    ...
    parser.add_argument("--root", type=Path,
                        default=repo / "Output/index/one_shot_bufferize_hivm_decompose_op")
    ...
    for row in rows:
        ...
        tuples.add(key)
        input_path = Path(row["after_cvpipelining_ir"])
        tree = Path(row["pass_tree"])
        if row["status"] != "complete":
            failures.append(f"incomplete tuple {key}")
        if not input_path.is_file() or sha256(input_path) != row["after_cvpipelining_sha256"]:
        ...
        previous = objects.setdefault(row["after_cvpipelining_sha256"], row)
    ...
    generator = Path(__file__).with_name("dump_one_shot_bufferize_hivm_decompose_op_oracles.py")
    for input_hash, row in selected:
        with tempfile.TemporaryDirectory(prefix="one-shot-bufferize-hivm-decompose-op-rerun-") as tmp:
            temp_root = Path(tmp)
            mini_summary = temp_root / "cvpipelining.tsv"
            with mini_summary.open("w", newline="") as stream:
                fields = ["adapter", "snapshot", "config_id", "status",
                          "after_cvpipelining_sha256", "after_cvpipelining_ir"]
                writer = csv.DictWriter(stream, fieldnames=fields, delimiter="\t")
                writer.writeheader()
                writer.writerow({name: row[name] for name in fields})
            result = subprocess.run([
                "python3", str(generator), "--cvpipelining-summary", str(mini_summary),
                "--tool", str(args.tool.resolve()), "--output-root", str(temp_root),
                "--no-resume"], stdout=subprocess.DEVNULL,
                stderr=subprocess.DEVNULL)
            rerun_tree = (temp_root / "experiments/one_shot_bufferize_hivm_decompose_op/objects" /
    ...
    if failures:
        ...
        print(f"ONE_SHOT_BUFFERIZE_HIVM_DECOMPOSE_OP_ORACLES=FAIL failures={len(failures)}")
        return 1
    print(f"ONE_SHOT_BUFFERIZE_HIVM_DECOMPOSE_OP_ORACLES=PASS tuples={len(rows)} "
