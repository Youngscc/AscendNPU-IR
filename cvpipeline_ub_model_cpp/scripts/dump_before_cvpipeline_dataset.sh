...
DATASET_ROOT="${MODULE_DIR}/data/before_cvpipeline"
WORK_ROOT="${REPO_ROOT}/Output/dataset_before_cvpipeline"
COMPILER="${BISHENGIR_COMPILER:-${REPO_ROOT}/build/bin/bishengir-compile}"
SUFFIX_TOOL="${CVPIPELINE_SUFFIX_TOOL:-${REPO_ROOT}/build/bin/bishengir-cvpipeline-suffix-compile}"
MAX_FILES=0

usage() {
  cat <<'EOF'
Usage: bash cvpipeline_ub_model_cpp/scripts/dump_before_cvpipeline_dataset.sh [options]

Generate the checked-in before-CVPipeline dataset from all source adapters.
...
bash "${REPO_ROOT}/tools/dump_cvpipeline_before_ir.sh" "${dump_args[@]}"

summary="${WORK_ROOT}/index/cvpipeline_compiler_default/summary.tsv"
...
printf 'adapter\tdump_status\tcompile_status\tsnapshot_count\tadapter_sha256\tsuffix_tool_sha256\tbefore_cvpipeline_files\n' >"${manifest}"
