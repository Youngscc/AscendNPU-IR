#!/usr/bin/env python3
"""Require byte-exact C1 SemanticIR equality and provenance."""
...
def main() -> int:
    ...
    if len(rows) != dataset.get("mapped_tuple_count") or len(rows) != dataset.get("c0_tuple_count"):
    ...
    for row in rows:
        ...
        after_ir = Path(row["after_cvpipeline_ir"])
        if (not after_ir.is_file() or
                sha256(after_ir) != row["after_cvpipeline_sha256"]):
            failures.append(f"stale C1 input {key}")
        if (row["oracle_tool_sha256"] != tool_hash or
                row["model_tool_sha256"] != model_hash):
            failures.append(f"stale tuple tool provenance {key}")
    unique = {row["after_cvpipeline_sha256"]: row for row in rows}
    ...
    for input_hash, row in unique.items():
        ...
        if args.rerun:
            with tempfile.TemporaryDirectory(prefix="c1-semantic-rerun-") as tmp:
                rerun = Path(tmp) / "model.tsv"
                result = subprocess.run([
                    str(args.model.resolve()), "--action=dump-c1-semantic-ir",
    ...
    if failures:
        ...
        print(f"C1_SEMANTIC_IR=FAIL failures={len(failures)}")
        return 1
    print(f"C1_SEMANTIC_IR=PASS tuples={len(rows)} "
