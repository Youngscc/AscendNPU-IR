#!/usr/bin/env python3
"""Validate the lightweight PlanMemory model against real PlanMemory."""
...
def main() -> int:
    ...
    failures: list[str] = []
    checked = 0
    with tempfile.TemporaryDirectory(prefix="cvub-plan-memory-") as temp_dir:
        temp = Path(temp_dir)
        for row_index, row in enumerate(rows):
            ...
            for seed in seeds:
                label = f"{row['after_cvpipelining_sha256']}/{row['config_id']}/{seed}"
                env = os.environ.copy()
                ...
                for kind in ("lifetime", "plan"):
                    ...
                    command = [
                        str(tool),
                        f"--action=model-plan-memory-structured-{kind}",
                ...
                checked += 1
    if failures:
        print(f"PLAN_MEMORY_MODEL=FAIL checked={checked} failures={len(failures)}")
        print("\n".join(failures[:100]))
        return 1
    print(f"PLAN_MEMORY_MODEL=PASS tuples={checked}")
    return 0
