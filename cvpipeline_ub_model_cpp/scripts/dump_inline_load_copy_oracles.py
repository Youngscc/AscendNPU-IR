#!/usr/bin/env python3
"""Capture native InlineLoadCopy before/after snapshots."""
...
def main() -> int:
    ...
    index_root = output_root / "index/inline_load_copy"
    object_root = output_root / "experiments/inline_load_copy/objects"
    ...
    fields = ["after_cvpipelining_sha256", "after_cvpipelining_ir", "status",
    ...
    with (index_root / "summary.tsv").open("w", newline="") as stream:
        ...
        writer.writeheader()
        for input_hash, input_path in selected.items():
            writer.writerow({"after_cvpipelining_sha256": input_hash,
                             "after_cvpipelining_ir": input_path,
                             "oracle_tool_sha256": tool_hash,
                             **states[input_hash]})
