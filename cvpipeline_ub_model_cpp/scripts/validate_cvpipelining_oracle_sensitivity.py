#!/usr/bin/env python3
"""Ensure the CVPipelining validator rejects oracle mutations."""
...
def main() -> int:
    ...
    validator = Path(__file__).with_name("validate_cvpipelining_oracles.py")
    mutations = ("input_sha256", "after_cvpipelining_sha256", "config_sha256")

    for field in mutations:
        with tempfile.TemporaryDirectory(
                prefix="cvpipelining-oracle-sensitivity-") as tmp:
            root = Path(tmp)
            ...
            if result.returncode == 0:
                ...
                return 1
    print(f"CVPIPELINING_ORACLE_SENSITIVITY=PASS mutations={len(mutations)}")
    return 0
