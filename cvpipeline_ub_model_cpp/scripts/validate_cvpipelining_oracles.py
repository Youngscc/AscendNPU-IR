#!/usr/bin/env python3
"""Validate CVPipelining oracle completeness and reproducibility."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--oracle-root", type=Path,
                        default=repo / "Output/index/cvpipelining")
...
def main() -> int:
    ...
    if not summary.is_file() or not dataset_file.is_file() or not tool.is_file():
        raise SystemExit(
            "[ERROR] missing CVPipelining summary, dataset manifest, or "
            "oracle tool")
    with summary.open(newline="") as stream:
        reader = csv.DictReader(stream, delimiter="\t")
        if not REQUIRED.issubset(reader.fieldnames or []):
            raise SystemExit(
                "[ERROR] CVPipelining summary schema is incomplete")
        rows = list(reader)
    ...
    for row in rows:
        ...
        paths = {name: Path(row[name]) for name in (
            "input", "after_cvpipelining_ir", "boundary_dump", "config_json", "command_json")}
        ...
        expected_hashes = {
            ...
            "after_cvpipelining_ir": row["after_cvpipelining_sha256"],
        ...
        if canonical_ir(paths["after_cvpipelining_ir"].read_bytes()) != canonical_ir(paths["boundary_dump"].read_bytes()):
    ...
    for row in selected:
        ...
        with tempfile.TemporaryDirectory(
                prefix="cvpipelining-oracle-rerun-") as tmp:
            tmp_path = Path(tmp)
    ...
    for row in rows[:args.full_suffix_cross_check]:
        key = (row["adapter"], row["snapshot"], row["config_id"])
        with tempfile.TemporaryDirectory(
                prefix="cvpipelining-oracle-full-") as tmp:
            tmp_path = Path(tmp)
    ...
    if failures:
        ...
        print(f"CVPIPELINING_ORACLE=FAIL failures={len(failures)}")
        return 1
    print(f"CVPIPELINING_ORACLE=PASS tuples={len(rows)} rerun={len(selected)} full_cross_check={args.full_suffix_cross_check}")
    return 0
