#!/usr/bin/env python3
"""Dump one generic-form before-CVPipeline object per unique D0 input."""
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        default=repo / "Output/index/d0_before_cvpipeline_generic")
    parser.add_argument("--limit", type=int, default=0)
    args = parser.parse_args()

    rows = list(csv.DictReader(args.c0_summary.open(), delimiter="\t"))
    ...
    manifest = {
        ...
        "c0_summary": str(args.c0_summary.resolve()),
    ...
    if failures:
        print("\n".join(failures[:20]))
        return 1
    print(f"D0_BEFORE_GENERIC=PASS objects={len(summary_rows)}")
    print(summary)
    return 0
