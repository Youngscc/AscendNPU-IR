#!/usr/bin/env python3
"""Compare modeled and direct PlanMemory input projections."""
...
def main() -> int:
    ...
    if args.only_hash:
        ...
        rows = [row for row in rows
                if row["after_cvpipelining_sha256"] in selected]
    if args.one_config_per_ir:
        unique_rows = {}
        for row in rows:
            unique_rows.setdefault(row["after_cvpipelining_sha256"], row)
    ...
    if args.artifacts:
        ...
        temporary = tempfile.TemporaryDirectory(prefix="cvub-plan-memory-input-")
        temp = Path(temporary.name)
    try:
        for index, row in enumerate(rows):
            label = f"{row['after_cvpipelining_sha256']}/{row['config_id']}"
            source = (repo / "Output/experiments/one_shot_bufferize_hivm_decompose_op/objects" /
                      row["after_cvpipelining_sha256"] / "pass_tree" /
            ...
            oracle_result = subprocess.run(
                [str(tool), "--action=dump-plan-memory-canonical-input-oracle",
            ...
            model_result = subprocess.run(
                [str(tool), "--action=model-plan-memory-canonical-input",
    ...
    if failures:
        print(f"PLAN_MEMORY_INPUT_PROJECTION=FAIL checked={checked} "
              f"failures={len(failures)}")
        displayed = failures if args.max_failures == 0 else failures[:args.max_failures]
        print("\n".join(displayed))
        return 1
    print(f"PLAN_MEMORY_INPUT_PROJECTION=PASS tuples={checked}")
    return 0
