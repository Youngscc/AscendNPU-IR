#!/usr/bin/env python3
"""Capture native MarkMultiBuffer snapshots for the strategy matrix."""
...
def run_one(tool: Path, input_hash: str, input_path: Path,
    ...
    return {
        "after_cvpipelining_sha256": input_hash,
        "after_cvpipelining_ir": str(input_path), "config_id": config_id,
...
def main() -> int:
    args = parse_args()
    with args.cvpipelining_summary.resolve().open(newline="") as stream:
    ...
    object_root = output / "experiments/mark_multi_buffer/objects"
    index_root = output / "index/mark_multi_buffer"
    ...
    results.sort(key=lambda row: (row["after_cvpipelining_sha256"],
                                  row["config_id"]))
    ...
    print(f"MARK_MULTI_BUFFER_ORACLES objects={len(selected)} "
