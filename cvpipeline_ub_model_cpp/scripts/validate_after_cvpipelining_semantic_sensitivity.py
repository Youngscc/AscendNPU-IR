#!/usr/bin/env python3
"""Ensure after-CVPipelining comparison rejects mutations."""
...
def main() -> int:
    ...
    repo = module.parent
    source = repo / "Output/index/after_cvpipelining_semantic_ir"
    validator = Path(__file__).with_name("validate_after_cvpipelining_semantic_ir.py")
    # name, operand identity, and memory effect columns.
    for column in (6, 8, 14):
        with tempfile.TemporaryDirectory(prefix="after-cvpipelining-semantic-sensitivity-") as tmp:
            root = Path(tmp)
            ...
            if not rows or fields is None:
                return 2
            target_hash = rows[0]["after_cvpipelining_sha256"]
            ...
            for row in rows:
                if row["after_cvpipelining_sha256"] == target_hash:
            ...
            if result.returncode == 0:
                print(f"[ERROR] after-CVPipelining validator accepted mutation in column {column}")
                return 1
    print("AFTER_CVPIPELINING_SEMANTIC_SENSITIVITY=PASS mutations=3")
    return 0
