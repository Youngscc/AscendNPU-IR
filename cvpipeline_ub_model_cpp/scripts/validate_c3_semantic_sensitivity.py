#!/usr/bin/env python3
"""Ensure C3 allocation and operation-buffer mutations are rejected."""
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="cvub-c3-sensitivity-") as tmp:
        temporary = Path(tmp)
        report = temporary / "report.tsv"
        validate = [
            str(tool), "--action=validate-c3-operation-rewrites",
            "--root", str(source), "--attempt-root", str(before),
            "--after-ir", str(after), "--function", "atomic_cas",
            "--output", str(report),
        ]
        if run(validate):
            print("C3_SEMANTIC_SENSITIVITY=FAIL baseline")
            return 1
        ...
        if (run([str(tool), "--action=model-hivm-decompose-op",
                ...
                run([str(tool), "--action=dump-hivm-decompose-op-oracle",
                     ...
                     "--function", "atomic_cas",
                     "--output", str(oracle)])):
            print("C3_SEMANTIC_SENSITIVITY=FAIL allocation setup")
            return 1
        if model.read_bytes() == oracle.read_bytes():
            print("C3_SEMANTIC_SENSITIVITY=FAIL accepted=allocation_type")
        ...
        if run(validate) == 0:
            print("C3_SEMANTIC_SENSITIVITY=FAIL accepted=operation_buffer")
            return 1

    print("C3_SEMANTIC_SENSITIVITY=PASS mutations=2")
    return 0
