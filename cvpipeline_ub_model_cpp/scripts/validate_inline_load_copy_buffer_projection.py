#!/usr/bin/env python3
"""Validate InlineLoadCopy buffer projection."""
...
def lcs_survivors(before: list[tuple[str, ...]],
    ...
    if len(survivors) != len(after):
        raise RuntimeError("InlineLoadCopy oracle allocation sequence is not a subsequence")
    return survivors
...
def main() -> int:
    ...
    parser.add_argument(
        "--inline-load-copy-objects", type=Path,
        default=repo / "Output/experiments/inline_load_copy/objects")
    parser.add_argument(
        "--tool", type=Path,
    ...
    inline_load_copy_objects = args.inline_load_copy_objects.resolve()
    tool = args.tool.resolve()
    failures: list[str] = []
    checked = function_pairs = changed_pairs = rewrites = buffers = 0
    with tempfile.TemporaryDirectory(prefix="cvub-inline-load-copy-") as temporary:
        temp = Path(temporary)
        for directory in sorted(inline_load_copy_objects.iterdir()):
            if not directory.is_dir():
                continue
            source = (alloc_extra_buffer_objects / directory.name / "pass_tree" /
            ...
            alloc_extra_buffer_oracle = prefix.with_suffix(
                ".alloc_extra_buffer.tsv")
            ...
            commands = (
                [str(tool), "--action=model-inline-load-copy-buffer-projection",
                 "--root", str(source), "--output", str(model)],
                [str(tool), "--action=model-inline-load-copy-semantic-ir",
                ...
                [str(tool), "--action=dump-alloc-extra-buffer-projection-oracle",
                 ...
                 "--output", str(alloc_extra_buffer_oracle)],
            ...
            alloc_extra_buffer_lines = [line for line in alloc_extra_buffer_oracle.read_text().splitlines()
                        if line.startswith("BUFFER\t")]
            if len(before) != len(alloc_extra_buffer_lines):
                failures.append(
                    f"{directory.name}: AllocExtraBuffer/before count "
                    f"{len(alloc_extra_buffer_lines)} != "
                    f"{len(before)}")
                continue
            try:
            ...
            oracle = "INLINE_LOAD_COPY_BUFFER_PROJECTION\t1\n" + "".join(
                line + "\n" for index, line in enumerate(alloc_extra_buffer_lines)
            ...
            for line in candidates.read_text().splitlines():
                ...
                oracle_signatures.append(
                    (fields[1], alloc_extra_buffer_lines[middle_ordinal].split("\t")[1],
                     alloc_extra_buffer_lines[destination_ordinal].split("\t")[1]))
