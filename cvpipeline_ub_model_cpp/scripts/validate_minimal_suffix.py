#!/usr/bin/env python3
"""Acceptance gate for the minimal suffix ending at PlanMemory."""
...
def main() -> int:
    ...
    input_projection = subprocess.run(
        ["python3", str(module / "scripts/validate_plan_memory_input_projection.py"),
         "--one-config-per-ir", "--max-failures=20"],
        text=True, capture_output=True)
    input_projection_text = input_projection.stdout.strip()
    if ("checked=165 failures=1" not in input_projection_text and
            input_projection.returncode != 0):
        print(input_projection_text)
        print(input_projection.stderr.strip())
        return 1

    rows = list(csv.DictReader(
        (repo / "Output/index/mark_multi_buffer/summary.tsv").open(),
        delimiter="\t"))
    unique: dict[str, dict[str, str]] = {}
    for row in rows:
        unique.setdefault(row["after_cvpipelining_sha256"], row)
    ...
    for digest, row in unique.items():
        source = (repo / "Output/experiments/one_shot_bufferize_hivm_decompose_op/objects" /
                  digest / "pass_tree/builtin_module_no-symbol-name" /
                  "0_one-shot-bufferize.mlir")
        command = [
            str(binary), "--debug", "--debug-entry=after-cvpipelining",
            f"--after-cvpipelining-ir={source}", "--random-seed=0", "--format=json",
    ...
    report = module / "output/reports/minimal_suffix.tsv"
    ...
    if failures or coverage < args.min_coverage:
        print(f"MINIMAL_SUFFIX=FAIL accepted={accepted} blocked={blocked} "
              f"total={total} coverage={coverage:.4f}")
        print("\n".join(failures[:20]))
        return 1
    print(f"MINIMAL_SUFFIX=PASS accepted={accepted} blocked={blocked} "
          f"total={total} coverage={coverage:.4f}")
    print(input_projection_text)
    return 0
