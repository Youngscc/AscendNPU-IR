#!/usr/bin/env python3
"""Validate MarkMultiBuffer state against native pass snapshots."""
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="cvub-mark-multi-buffer-") as temporary:
        temp = Path(temporary)
        for index, row in enumerate(rows):
            if row["status"] != "complete":
                failures.append(f"{row['after_cvpipelining_sha256']}/{row['config_id']}: incomplete")
                continue
            ...
            command = [str(tool), "--action=model-mark-multi-buffer-projection",
    ...
    if failures:
        print(f"MARK_MULTI_BUFFER=FAIL tuples={checked} failures={len(failures)}")
        print("\n".join(failures[:30]))
        return 1
    print(f"MARK_MULTI_BUFFER=PASS tuples={checked} marks={marks} "
