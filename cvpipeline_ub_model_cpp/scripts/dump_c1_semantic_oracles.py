#!/usr/bin/env python3
"""Generate C1 SemanticIR oracles from real MLIR objects and model output."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output-root", type=Path,
                        default=repo / "Output/index/c1_semantic_ir")
...
def main() -> int:
    ...
    for input_hash, input_path in selected.items():
        ...
        if not reusable:
            command = [
                ...
                f"--dump-c1-semantic-oracle={oracle}",
                f"--dump-c1-generic-ir={generic}", "-o", str(suffix_output),
            ]
        ...
        model_result = subprocess.run([
            str(model), "--action=dump-c1-semantic-ir", f"--root={generic}",
            f"--output={modeled}",
    ...
    manifest = {
        "schema_version": 1,
        "c0_summary": str(summary_path),
        "c0_summary_sha256": sha256(summary_path),
        "c0_tuple_count": len(rows),
