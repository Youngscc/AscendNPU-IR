# CVPipeline UB Model Data

This directory keeps source adapters and their reproducible model boundary
inputs together under one development module.
...
Keep adapters directly under `adapter/`. Generate or refresh the paired
...
compiler revision and do not produce a before-CVPipeline snapshot:
...
The current dataset contains 174 adapters, 171 generated snapshots, and three
recorded failures. The 171 snapshots contain 160 distinct byte sequences; all
adapter-to-snapshot relationships are retained even when two adapters lower to
the same boundary IR.
