...
The product branch includes two self-contained before-CVPipeline generic IR
examples:
...
Use `demo_vector_add_before_cvpipeline.mlir` for a small smoke test and
`demo_randn_before_cvpipeline.mlir` for a more representative UB workspace
example. The one-command demo uses the randn example by default; select
another input with `--pre-cvpipeline-ir`.
...
The demo script also writes a fresh HTML snapshot with the latest JSON embedded:
...
```bash
FORCE_REBUILD=1 bash cvpipeline_ub_model_cpp/build.sh
```
