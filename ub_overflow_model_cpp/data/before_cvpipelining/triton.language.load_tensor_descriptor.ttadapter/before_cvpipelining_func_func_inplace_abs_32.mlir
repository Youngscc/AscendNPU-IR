...
"builtin.module"() ({
  ...
  ^bb0(%arg0: i64, %arg1: memref<?xi8>, %arg2: memref<?xi8>, %arg3: memref<?xf32>, %arg4: i32, %arg5: i32, %arg6: i32, %arg7: i32, %arg8: i32):
    ...
    %14 = "arith.muli"(%13, %3) <{overflowFlags = #arith.overflow<none>}> : (i32, i32) -> i32
    %15 = "arith.muli"(%10, %3) <{overflowFlags = #arith.overflow<none>}> : (i32, i32) -> i32
    %16 = "arith.maxsi"(%14, %2) : (i32, i32) -> i32
    %17 = "arith.index_cast"(%16) : (i32) -> index
    %18 = "arith.maxsi"(%15, %2) : (i32, i32) -> i32
    %19 = "arith.index_cast"(%18) : (i32) -> index
    %20 = "arith.index_cast"(%arg5) : (i32) -> index
    %21 = "affine.apply"(%17, %20) <{map = #map}> : (index, index) -> index
    %22 = "arith.index_cast"(%arg4) : (i32) -> index
    %23 = "affine.apply"(%21, %19) <{map = #map1}> : (index, index) -> index
    ...
    %26 = "affine.apply"(%23, %20) <{map = #map2}> : (index, index) -> index
    %27 = "affine.apply"(%22, %26) <{map = #map3}> : (index, index) -> index
    %28 = "affine.max"(%27) <{map = #map4}> : (index) -> index
    %29 = "affine.min"(%28) <{map = #map5}> : (index) -> index
    %30 = "affine.apply"(%23, %20) <{map = #map6}> : (index, index) -> index
    %31 = "affine.apply"(%20, %30) <{map = #map3}> : (index, index) -> index
    %32 = "affine.max"(%31) <{map = #map4}> : (index) -> index
    %33 = "affine.min"(%32) <{map = #map5}> : (index) -> index
    %34 = "arith.subi"(%2, %14) <{overflowFlags = #arith.overflow<none>}> : (i32, i32) -> i32
    %35 = "arith.maxsi"(%34, %2) : (i32, i32) -> i32
    %36 = "arith.index_cast"(%35) : (i32) -> index
    %37 = "affine.min"(%36, %29) <{map = #map7}> : (index, index) -> index
    %38 = "affine.apply"(%29, %37) <{map = #map3}> : (index, index) -> index
    %39 = "arith.subi"(%2, %15) <{overflowFlags = #arith.overflow<none>}> : (i32, i32) -> i32
    %40 = "arith.maxsi"(%39, %2) : (i32, i32) -> i32
    %41 = "arith.index_cast"(%40) : (i32) -> index
    %42 = "affine.min"(%41, %33) <{map = #map7}> : (index, index) -> index
    %43 = "affine.apply"(%33, %42) <{map = #map3}> : (index, index) -> index
    ...
    %49 = "arith.remui"(%42, %4) : (index, index) -> index
    "hivm.hir.load"(%47, %48, %0, %49, %46) <{eviction_policy = #hivm.eviction_policy<EvictFirst>, init_out_buffer = true, may_implicit_transpose_with_last_axis = false, operandSegmentSizes = array<i32: 1, 1, 1, 1, 0, 1>, pad_mode = #hivm.padmode<PadValue>, tcoretype = #hivm.tcore_type<VECTOR>}> : (memref<?x?xf32, strided<[?, 1], offset: ?>>, memref<?x?xf32, strided<[32, 1], offset: ?>>, f32, index, i1) -> ()
    %50 = "bufferization.to_tensor"(%25) <{restrict, writable}> : (memref<32x32xf32>) -> tensor<32x32xf32>
    %51 = "tensor.empty"() : () -> tensor<32x32xf32>
    %52 = "hivm.hir.vabs"(%50, %51) <{broadcast = array<i64>, operandSegmentSizes = array<i32: 1, 1, 0>, transpose = array<i64>}> : (tensor<32x32xf32>, tensor<32x32xf32>) -> tensor<32x32xf32>
    %53 = "tensor.extract_slice"(%52, %37, %42, %38, %43) <{operandSegmentSizes = array<i32: 1, 2, 2, 0>, static_offsets = array<i64: -9223372036854775808, -9223372036854775808>, static_sizes = array<i64: -9223372036854775808, -9223372036854775808>, static_strides = array<i64: 1, 1>}> : (tensor<32x32xf32>, index, index, index, index) -> tensor<?x?xf32>
    "hivm.hir.store"(%53, %47) : (tensor<?x?xf32>, memref<?x?xf32, strided<[?, 1], offset: ?>>) -> ()
    "func.return"() : () -> ()
