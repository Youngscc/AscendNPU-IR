#!/usr/bin/env python3
"""Compare the lightweight ArithToAffine stage with native BiSheng."""
...
def verify_one(
    ...
    single = run([str(runner), "--apply-arith-to-affine", str(after_outer)])
    oracle = run([str(runner), str(after_arith)])
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="arith-to-affine-") as temporary:
...
if __name__ == "__main__":
    try:
        ...
        print(f"verify_arith_to_affine_pipeline: {error}", file=os.sys.stderr)
        raise SystemExit(2)
