...
struct PlanRecordModel {
  ...
  MemoryBoundModel *firstMemBound = nullptr;
...
inline void ResetMemPlanState(MemPlanStateModel &state) {
  ...
  state.firstBufferEntry2RelationOtherBufferEntry.clear();
  state.entryById.clear();
  state.conflictProfileByBuffer.clear();
  state.conflictProfileCount = 0;
  // clear() preserves vector capacity, so retry attempts reuse the largest
  // conflict/lookup allocation observed by earlier seeds.
  state.lifeConflict.clear();
  state.pipeConflict.clear();
  state.inplacableConflict.clear();
}
...
inline PlanRecordModel RollbackOutline(PlanRecHisModel &history,
  ...
  auto it = std::find_if(outline.begin(), outline.end(),
                         [&](MemoryBoundModel &bound) {
                           return &bound == record.firstMemBound;
                         });
  if (record.headed) {
    --it;
    it = outline.erase(it);
