...
private:
  ...
  planMemoryTypes(const std::vector<std::string> &values) const {
    std::vector<std::string> types;
    types.reserve(values.size());
    for (const std::string &value : values) {
      auto type = namedValueTypes.find(value);
      types.push_back(type == namedValueTypes.end() ? "memref<1xi8>"
                                                     : type->second);
    }
    return types;
  }
  ...
  bool emitSourceDestinationStyleAccess(
    ...
    std::vector<std::string> operandNames = inputNames;
    operandNames.insert(operandNames.end(), outputNames.begin(),
                        outputNames.end());
    operandNames.insert(operandNames.end(), scalarOperands.begin(),
                        scalarOperands.end());
    SetOperationValueLists(operation, {}, std::move(operandNames));
    SetOperationAccessGroups(operation, std::move(inputNames),
  ...
  void emitAccess(const GenericOperation &source,
    ...
    if (emitSourceDestinationStyleAccess(operation, source, event,
      ...
      if (!temporaries.empty()) {
        ...
        operation.materializedOperands.insert(
            operation.materializedOperands.end(), temporaries.begin(),
            temporaries.end());
        operation.materializedTemporaries = temporaries;
      ...
      std::vector<std::string> operands{scalar, memory};
      if (source.name == "tensor.from_elements") {
        const GenericOperation *function = EnclosingFunction(logical, source);
        auto functionConstants =
        ...
        const std::string indexName = scalarValueName(index->second);
        operation.text += ", " + indexName;
        operands.push_back(indexName);
      } else {
        for (size_t index = 2; index < source.operands.size(); ++index) {
          operands.push_back(scalarValueName(source.operands[index]));
          operation.text += ", " + operands.back();
        }
      }
      if (namedValueTypes.count(memory) != 0)
        operation.text += " : " + namedValueTypes.at(memory);
      SetOperationValueLists(operation, {}, std::move(operands));
      SetOperationAccessGroups(operation, {}, {}, {}, {});
      ...
      std::vector<std::string> operands{memory};
      ...
      if (namedValueTypes.count(memory) != 0)
        operation.text += " : " + namedValueTypes.at(memory);
      SetOperationValueLists(operation, {scalar}, std::move(operands));
      SetOperationAccessGroups(operation, {}, {}, {}, {});
      ...
      SetOperationValueLists(
          operation, {resultName},
          {scalarValueName(source.operands[0]), valueName(source.operands[1]),
           valueName(source.operands[2])});
      SetOperationAccessGroups(operation, {}, {}, {}, {});
      ...
      operands.insert(operands.end(), temporaries.begin(), temporaries.end());
      SetOperationValueLists(operation, {}, std::move(operands));
      SetOperationAccessGroups(operation, reads, planMemoryTypes(reads),
                               writes, planMemoryTypes(writes), temporaries,
                               planMemoryTypes(temporaries));
    } else {
      std::vector<std::string> operands = reads;
      operands.insert(operands.end(), writes.begin(), writes.end());
  ...
  void emitControlStart(const GenericOperation &source) {
    ...
    operation.materializedControlFlow = true;
    std::ostringstream text;
    ...
    if (source.name == "scf.for") {
      ...
      operation.materializedForInduction = inductionVariable;
      const std::string lower =
          source.operands.empty() ? "%lb"
                                  : scalarValueName(source.operands[0]);
      const std::string upper =
          source.operands.size() < 2 ? "%ub"
                                     : scalarValueName(source.operands[1]);
      const std::string step =
          source.operands.size() < 3 ? "%step"
                                     : scalarValueName(source.operands[2]);
      typedOperands.insert(typedOperands.end(), {lower, upper, step});
      text << "scf.for " << inductionVariable << " = " << lower << " to "
           << upper << " step " << step;
      ...
      if (hasScalarIterArg && !source.regions.empty()) {
        ...
        for (size_t index = 3; index < source.operands.size(); ++index) {
          ...
          const std::string iterArgument =
              PlanMemoryValueName(block.arguments.at(argument));
          operation.materializedIterArgPairs.emplace_back(iterArgument, init);
          typedOperands.push_back(init);
          text << iterArgument << " = " << init;
          firstIterArg = false;
        }
        text << ')';
      ...
      for (size_t index = 0; index < source.operands.size(); ++index) {
        if (index)
          text << ", ";
        const std::string argument = PlanMemoryValueName(
            -source.id * 1000 - static_cast<int>(index) - 1);
        const std::string init = valueName(source.operands[index]);
        operation.materializedWhileInitArgPairs.emplace_back(argument, init);
        typedOperands.push_back(init);
        text << argument << " = " << init;
      }
      text << ')';
      ...
      typedOperands.push_back(condition);
      text << "scf.if " << condition;
      if (hasResult) {
        text << " -> (";
        bool firstType = true;
        for (size_t index = 0; index < source.results.size(); ++index) {
    ...
    SetOperationValueLists(operation, std::move(typedResults),
                           std::move(typedOperands));
    result.operations.push_back(std::move(operation));
  }
  ...
  void emitPassthrough(const GenericOperation &source) {
    ...
    text << name;
    std::vector<std::string> typedOperands;
    typedOperands.reserve(source.operands.size());
    if (!affineName.empty()) {
      ...
      for (size_t index = 0; index < source.operands.size(); ++index) {
        ...
        if (source.name == "hivm.hir.bitcast") {
          ...
          if (buffer != valueBuffers.end()) {
            ...
            typedOperands.push_back(
                stable == stableBufferAliases.end()
                    ? bufferNames.at(buffer->second)
                    : stable->second);
          } else {
            typedOperands.push_back(valueName(source.operands[index]));
          }
        } else {
          typedOperands.push_back(passthroughOperandName(index));
        }
        text << typedOperands.back();
        firstOperand = false;
      }
    }
    if (source.name == "annotation.mark") {
