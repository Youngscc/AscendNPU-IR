#ifndef CVPIPELINE_UB_MODEL_CPP_IR_POST_PIPELINE_IR_UTILS_HPP
#define CVPIPELINE_UB_MODEL_CPP_IR_POST_PIPELINE_IR_UTILS_HPP
...
inline void RemapValues(std::vector<int> &values,
                        const std::map<int, int> &replacement) {
  for (int &value : values) {
    const auto mapped = replacement.find(value);
    if (mapped != replacement.end())
      value = mapped->second;
  }
}
...
inline void ValidateGenericModule(const GenericModule &module) {
  ...
  for (size_t index = 1; index < operationParentLinks.size(); ++index)
    if (operationParentLinks[index] != 1)
      throw std::runtime_error(
          "generic IR validation: operation " + std::to_string(index) +
          " (" + module.operations[index].name + ") has " +
          std::to_string(operationParentLinks[index]) +
          " parent links; expected exactly one");
  ...
  for (const GenericRegion &region : module.regions) {
    ...
    if (region.ordinal < 0 ||
      ...
      throw std::runtime_error(
          "generic IR validation: invalid region ordinal for region " +
          std::to_string(region.id) + " (parent operation " +
          std::to_string(region.parentOperation) + ", ordinal " +
          std::to_string(region.ordinal) + ")");
  ...
  const auto enclosingFunction = [&](int operationId) {
  ...
  for (const GenericOperation &operation : module.operations) {
    ...
    for (size_t index = 0; index < operation.operands.size(); ++index) {
      ...
      if (definition == definitions.end())
        continue;
      const int definitionAnchor = definition->second.operationId >= 0
                                       ? definition->second.operationId
                                       : module.regions
                                             .at(static_cast<size_t>(
                                                 definition->second.regionId))
                                             .parentOperation;
      if (enclosingFunction(definitionAnchor) !=
          enclosingFunction(operation.id))
        throw std::runtime_error(
            "generic IR validation: cross-function SSA reference at " +
            std::to_string(operation.id));
