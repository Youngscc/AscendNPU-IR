...
struct OperationRecord {
  ...
  bool materializedAccessGroups = false;
  std::vector<std::string> materializedInputs;
  std::vector<std::string> materializedInputTypes;
  std::vector<std::string> materializedOutputs;
  std::vector<std::string> materializedOutputTypes;
  std::vector<std::string> materializedTemporaries;
...
inline void RematerializeOperationValueLists(OperationRecord &operation) {
  operation.materializedValueLists = false;
  operation.materializedResults.clear();
  operation.materializedOperands.clear();
  MaterializeOperationValueLists(operation);
}

inline void SetOperationValueLists(OperationRecord &operation,
...
inline void SetOperationAccessGroups(
    OperationRecord &operation, std::vector<std::string> inputs,
    std::vector<std::string> inputTypes, std::vector<std::string> outputs,
    std::vector<std::string> outputTypes,
    std::vector<std::string> temporaries = {},
    std::vector<std::string> temporaryTypes = {}) {
  operation.materializedInputs = std::move(inputs);
  operation.materializedInputTypes = std::move(inputTypes);
  operation.materializedOutputs = std::move(outputs);
  operation.materializedOutputTypes = std::move(outputTypes);
  operation.materializedTemporaries = std::move(temporaries);
  operation.materializedTemporaryTypes = std::move(temporaryTypes);
  operation.materializedAccessGroups = true;
}

inline const std::vector<std::string> *
MaterializedOperationGroupNames(const OperationRecord &operation,
                                const std::string &group) {
  if (!operation.materializedAccessGroups)
    return nullptr;
  if (group == "ins")
    return &operation.materializedInputs;
  if (group == "outs")
    return &operation.materializedOutputs;
  if (group == "temp_buffer" || group == "tmps")
    return &operation.materializedTemporaries;
  return nullptr;
}

inline const std::vector<std::string> *
MaterializedOperationGroupTypes(const OperationRecord &operation,
                                const std::string &group) {
  if (!operation.materializedAccessGroups)
    return nullptr;
  if (group == "ins")
    return &operation.materializedInputTypes;
  if (group == "outs")
    return &operation.materializedOutputTypes;
  if (group == "temp_buffer" || group == "tmps")
    return &operation.materializedTemporaryTypes;
  return nullptr;
}

inline std::vector<std::string>
OperationGroupNames(const OperationRecord &operation,
                    const std::string &group) {
  if (const std::vector<std::string> *materialized =
          MaterializedOperationGroupNames(operation, group))
    return *materialized;
  return extractGroupSSAs(operation.text, group);
}

inline std::optional<std::string>
OperationForInductionVariable(const OperationRecord &operation) {
  if (operation.materializedControlFlow)
    return operation.materializedForInduction.empty()
               ? std::nullopt
               : std::optional<std::string>(
                     operation.materializedForInduction);
  return extractForInductionVariable(operation.text);
}

inline std::vector<std::pair<std::string, std::string>>
OperationIterArgPairs(const OperationRecord &operation) {
  return operation.materializedControlFlow
             ? operation.materializedIterArgPairs
             : extractIterArgPairs(operation.text);
}

inline std::vector<std::pair<std::string, std::string>>
OperationWhileInitArgPairs(const OperationRecord &operation) {
  return operation.materializedControlFlow
             ? operation.materializedWhileInitArgPairs
             : extractWhileInitArgPairs(operation.text);
}

inline void ReplaceOperationSSAUse(OperationRecord &operation,
  ...
  if (!operation.materializedValueLists)
    return;
  for (std::string &result : operation.materializedResults)
    if (result == from)
      result = to;
  for (std::string &operand : operation.materializedOperands)
    if (operand == from)
      operand = to;
  if (operation.materializedAccessGroups)
    for (std::vector<std::string> *group : {
             &operation.materializedInputs, &operation.materializedOutputs,
             &operation.materializedTemporaries})
      for (std::string &value : *group)
        if (value == from)
          value = to;
  if (operation.materializedControlFlow) {
    if (operation.materializedForInduction == from)
      operation.materializedForInduction = to;
    for (auto *pairs : {&operation.materializedIterArgPairs,
                        &operation.materializedWhileInitArgPairs})
      for (auto &[argument, init] : *pairs) {
        if (argument == from)
          argument = to;
        if (init == from)
          init = to;
...
inline void ReplaceOperationSSAUsesInMapOrder(
  ...
  if (!operation.materializedValueLists)
    return;
  for (std::string &result : operation.materializedResults)
    result = resolveSSAUseInMapOrder(std::move(result), aliases);
  for (std::string &operand : operation.materializedOperands)
  ...
  if (operation.materializedAccessGroups)
    for (std::vector<std::string> *group : {
             ...
             &operation.materializedTemporaries})
      for (std::string &value : *group)
        value = resolveSSAUseInMapOrder(std::move(value), aliases);
  if (operation.materializedControlFlow) {
    operation.materializedForInduction = resolveSSAUseInMapOrder(
        std::move(operation.materializedForInduction), aliases);
    for (auto *pairs : {&operation.materializedIterArgPairs,
                        &operation.materializedWhileInitArgPairs})
      for (auto &[argument, init] : *pairs) {
        argument = resolveSSAUseInMapOrder(std::move(argument), aliases);
        init = resolveSSAUseInMapOrder(std::move(init), aliases);
      }
  }
}
