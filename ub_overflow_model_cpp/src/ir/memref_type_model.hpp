...
inline bool IsIdentityStridedLayoutAttr(const MemRefTypeModel &type) {
  // StridedLayoutAttr::isIdentity() delegates to its affine map. A strided
  // linear layout has one affine-map result, so it can only be the identity
  // map for a rank-one, unit-stride, zero-offset memref.
  return type.hasStridedLayout && type.shape.size() == 1 &&
         type.strides.size() == 1 && type.strides.front() &&
         *type.strides.front() == 1 && type.offset == 0;
}
...
GetContiguousAxes(const std::vector<MemRefTypeModel> &types) {
  ...
  for (const MemRefTypeModel &type : types) {
    ...
    if (!type.hasStridedLayout || IsIdentityStridedLayoutAttr(type))
      continue;
