...
inline bool IsDestinationStyleOp(const std::string &opName) {
  static const std::set<std::string> operations = {
      ...
      "hivm.hir.custom",         "hivm.hir.custom_macro",
      "hivm.hir.fixpipe",
      "hivm.hir.gather_load",    "hivm.hir.indirect_store",
      "hivm.hir.l12ub",          "hivm.hir.load",
      "hivm.hir.matmul",
      ...
      "hivm.hir.vln",            "hivm.hir.visinf",
      "hivm.hir.visnan",         "hivm.hir.vmax",
...
// HIVMVectorOps.td derives every vector operation from HIVM_VectorOp, whose
// base traits include AlwaysSpeculatable. Keep this tied to the reviewed
// DestinationStyleOp registry so an unknown `hivm.hir.v*` operation is not
// accepted implicitly.
inline bool IsHIVMVectorOp(const std::string &opName) {
  return IsDestinationStyleOp(opName) &&
         opName.rfind("hivm.hir.v", 0) == 0;
}
...
inline bool IsSinglePipeOp(const std::string &opName) {
  static const std::set<std::string> operations = {
      ...
      "hivm.hir.visinf",        "hivm.hir.visnan",
...
inline bool IsElementwiseNaryOp(const std::string &opName) {
  static const std::set<std::string> operations = {
      ...
      "hivm.hir.visinf",   "hivm.hir.visnan",  "hivm.hir.vln",
      "hivm.hir.vmax",     "hivm.hir.vmin",
